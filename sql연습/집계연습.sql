-- 1) 집계(통계)쿼리 : select 절에 통계함수(avg, max, min, count, sum, stddev, variance, ...)
...
select emp_no, salary, from_date, to_date from salaries where emp_no = '10060';
...
-- 집계함수가 있다면 1column, 1row
-- having은 group by 다음에
-- 예제 : 사원별 평균 월급이 60,000 달러 이상인 사원의 사번과 평균 월급을 출력 하세요.
select emp_no, avg(salary) as avg_salary from salaries group by emp_no having avg_salary >= 60000;

-- 6) order by
--    order by는 항상 맨 마지막 출력전에 한다. alias 된 컬럼명은 '' 을 붙히지 않는다
select emp_no, avg(salary) as avg_salary from salaries group by emp_no having avg_salary >= 60000 order by avg_salary;
